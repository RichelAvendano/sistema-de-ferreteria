<?php 
    ...
    if(isset($busqueda_max) && $busqueda_max != "" && isset($busqueda_min) && $busqueda_min != ""){

        intval($busqueda_max);
        intval($busqueda_min);

        $consulta_datos = "SELECT producto.producto_id,producto.producto_nombre,producto.producto_precio,producto.producto_stock,producto.producto_foto,categoria.categoria_nombre FROM producto INNER JOIN categoria ON producto.categoria_id = categoria.categoria_id WHERE producto_" . $search_op . " >= " . $busqueda_min . " AND producto_" . $search_op . " <= " . $busqueda_max . " ORDER BY producto.producto_id ASC LIMIT $inicio, $registros";

        $consulta_total = "SELECT COUNT(producto_id) FROM producto INNER JOIN categoria ON producto.categoria_id = categoria.categoria_id WHERE producto_" . $search_op . " >= " . $busqueda_min . " AND producto_" . $search_op . " <= " . $busqueda_max;
        

    }else if( isset($_SESSION['rango_valor']) && $_SESSION['rango_valor'] != "rango"){
        
        if($_SESSION['rango_valor'] != "100"){
            $partes = explode(" ", $_SESSION['rango_valor']);
            // Convertir las partes a números
            $num_min = floatval($partes[0]);
            $num_max = floatval($partes[1]);

            $consulta_datos = "SELECT producto.producto_id,producto.producto_nombre,producto.producto_precio,producto.producto_stock,producto.producto_foto,categoria.categoria_nombre FROM producto INNER JOIN categoria ON producto.categoria_id = categoria.categoria_id WHERE producto_" . $search_op . " >= " . $num_min . " AND producto_" . $search_op . " <= " . $num_max . " ORDER BY producto.producto_id ASC LIMIT $inicio, $registros";

            $consulta_total = "SELECT COUNT(producto_id) FROM producto INNER JOIN categoria ON producto.categoria_id = categoria.categoria_id WHERE producto_" . $search_op . " >= " . $num_min . " AND producto_" . $search_op . " <= " . $num_max;

        }else{
            $rango = $_SESSION['rango_valor'];
            $rango = (int) $rango;
            $consulta_datos = "SELECT producto.producto_id,producto.producto_nombre,producto.producto_precio,producto.producto_stock,producto.producto_foto,categoria.categoria_nombre FROM producto INNER JOIN categoria ON producto.categoria_id = categoria.categoria_id WHERE producto_" . $search_op . " >= " . $rango . " ORDER BY producto.producto_id ASC LIMIT $inicio, $registros";

            $consulta_total = "SELECT COUNT(producto_id) FROM producto INNER JOIN categoria ON producto.categoria_id = categoria.categoria_id WHERE producto_" . $search_op . " >= " . $rango;

        }
 
    }else{     
        $consulta_datos="SELECT producto.producto_id,producto.producto_nombre,producto.producto_precio,producto.producto_stock,producto.producto_foto,categoria.categoria_nombre FROM producto INNER JOIN categoria ON producto.categoria_id = categoria.categoria_id ORDER BY producto.producto_id ASC LIMIT $inicio, $registros";
            ...
            <table class="table table is-bordered is-fullwidth table-custom-bordered ">
                <thead>
                    <tr class="has-background-link-90">
                        ...
                        <th class="has-text-black" style = "text-align: center">Categoria</th>
